...
@app.get("/inicio")
def home():
    # Se consulta Supabase en cada carga. Los colores se calculan con la fecha
...
def actualizar_estatus_taller_api(nosiniestro):
    ...
    if estatus not in valores_validos:
        return redirect("/")
    # Cambiar etapa reinicia el contador de días de la etapa en Supabase.
...
@app.get("/estado-supabase")
def estado_supabase():
    return jsonify({"supabase": "conectado" if ensure_connection() else "desconectado"})
